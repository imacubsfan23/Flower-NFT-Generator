...
class GIFCreator:
    ...
    def record(self, draw_func: Callable = None, **options):
        ...
        def wrap():
            self.draw()
            ontimer(self._stop, options.get('end_after', 0))

        wrap_draw = functools.wraps(self.draw)(wrap)

        try:
            # https://blog.csdn.net/lingyu_me/article/details/105400510
            reset()  # Does a clear() and then resets this turtle's state (i.e. direction, position etc.)
        except Terminator:
            reset()
    ...
    def _save(self):
        if self.__is_running:
            ...
            if not output_file.exists():
                getcanvas().postscript(file=output_file)  # 0001.eps, 0002.eps ...
            self.__counter += 1
            ontimer(self._save, t=self.duration)  # trigger only once, so we need to set it again.
#-----------------------------------------------------------------------------------------------------------------------------------
init(gs_windows_binary=r'C:\Program Files\gs\gs9.55.0\bin\gswin64c')

rgb = [random.randint(0,1),random.randint(0,1),random.randint(0,1)]
base = 6
turns = 10
delta = 100
theta = 360/base
...
def cg(active=rgb):#color generator
    return '#{:02x}{:02x}{:02x}'.format(r() if active[0] else 0, r() if active[1] else 0, r() if active[2] else 0)

def generate_value(maximum = base):
    value = []
    for i in range(turns):
        value.append(random.randint(0,maximum-1))
    return value
...
for i in range(turtle_count):
    ...
    seed_colors.append(cg())
    turtles[i].speed(0)
    turtles[i].begin_fill()
    turtles[i].hideturtle()
    turtles[i].color(seed_colors[i])
    turtles[i].left(360 / turtle_count * i)

gif_name = ''.join([str(n) for n in rgb]) + '_' + ''.join([str(n) for n in value])
...
if rgb != [0,0,0]:
    ...
    if rgb == [1,1,0]:
        end_i = 4
    gif_name += '_' + ''.join([str(n[start_i:end_i]) for n in seed_colors])


def draw_flower(values=value):    
    for v in values:
        for turt in turtles:
            turt.forward(delta)
            turt.left(theta * v)
...
if __name__ == '__main__':
    gif_demo = GIFCreator(gif_name)
    gif_demo.record(draw_flower, fps=10, end_after=1000)
